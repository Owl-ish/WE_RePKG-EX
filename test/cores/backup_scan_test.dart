...
void main() {
  ...
  group('copyStandings', () {
    ...
    // Workshop comparisons are shallow; MyProjects comparisons include subfolders.
    ...
    // Identical contents can have different timestamps.
    ...
    // Use enough folders to cross batch boundaries.
  ...
  group('readCardFaces', () {
    ...
    // Check each library uses its own backup metadata.
    test(
      ...
      () async {
        ...
        // Conflicting metadata exposes reads from the wrong folder.
        project(
    ...
    // Invalid metadata must not remove the backup card.
    test(
      ...
      () async {
        ...
        // Use a non-string title to exercise invalid metadata handling.
    ...
    test('a face carries the type, the rating and a date', () async {
      ...
      final CardFace face =
          faces[const BackupCard(WallpaperLibrary.myProjects, 'alpha')]!;
      // Match Extraction's lowercase type values.
      expect(face.type, 'scene');
      expect(face.rating, 'mature');
      // Use project.json's timestamp, not the folder's or the scan time.
      expect(
        face.modified,
...
''';
  ...
  group('workshopVersions', () {
    ...
    // The ACF display setting must not disable version detection.
    ...
    // Unreadable ACF data warns; a valid empty library does not.
    ...
    // Malformed ACF input can return an empty map without throwing.
  ...
  group('scanBackup', () {
    ...
    // Use literal paths so the test does not derive expectations from production code.
    ...
    /// Create a shader-cache-only folder.
    ...
    group('a folder Steam left behind', () {
      ...
      // Preserve the backup card when the live folder contains only junk.
      ...
      // Nested content may still be recoverable without a loadable top-level project.
      ...
      // Only the MyProjects backup contains usable content.
    ...
    test('the scan reports what it is doing and how far it has got', () async {
      ...
      // Require an intermediate progress report, not just completion.
      expect(
    ...
    // MyProjects comparison must work without saved records.
    ...
    // Ignore rebuilt shaders on both sides.
    ...
    // Windows folder matching must tolerate case differences.
    ...
    // Missing folders must be reported, not treated as empty libraries.
